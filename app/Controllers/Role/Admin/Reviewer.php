...
class Reviewer extends BaseController
{
    ...
    public function index()
    {
        try {
            ...
            return redirect()->to('admin/reviewer')->with('error', 'Terjadi kesalahan saat memuat data reviewer.');
        }
    }
    ...
    private function getReviewerPerformance($reviewerId)
    {
        ...
        $pendingReviews  = $this->reviewModel->where('id_reviewer', $reviewerId)->where('keputusan', 'pending')->countAllResults();
        $completedReviews= $totalReviews - $pendingReviews;
        $acceptedReviews = $this->reviewModel->where('id_reviewer', $reviewerId)->where('keputusan', 'diterima')->countAllResults();
        $rejectedReviews = $this->reviewModel->where('id_reviewer', $reviewerId)->where('keputusan', 'ditolak')->countAllResults();
        $revisionReviews = $this->reviewModel->where('id_reviewer', $reviewerId)->where('keputusan', 'revisi')->countAllResults();
    ...
    private function calculateAverageReviewTime($reviewerId)
    {
        ...
        foreach ($reviews as $r) {
            $u = strtotime($r['tanggal_upload']);
            $v = strtotime($r['tanggal_review']);
            if ($v > $u) {
                $totalDays += ceil(($v - $u) / 86400);
                $valid++;
            }
        }

        return $valid > 0 ? round($totalDays / $valid, 1) : 0;
    }
    ...
    private function getReviewerStatistics()
    {
        ...
        $activeReviewers = $this->userModel->where('role', 'reviewer')->where('status', 'aktif')->countAllResults();
    ...
    public function store()
    {
        ...
        try {
            ...
            foreach ($categories as $categoryId) {
                if (!$this->reviewerKategoriModel->save([
    ...
    public function toggleStatus($id)
    {
        ...
        try {
            if ($this->userModel->update($id, ['status' => $newStatus])) {
                ...
                $msg = $newStatus === 'aktif' ? 'Reviewer berhasil diaktifkan!' : 'Reviewer berhasil dinonaktifkan!';
                return redirect()->to('admin/reviewer')->with('success', $msg);
            }
            return redirect()->to('admin/reviewer')->with('error', 'Gagal mengubah status reviewer.');
        } catch (\Exception $e) {
            log_message('error', 'Toggle reviewer status error: ' . $e->getMessage());
            return redirect()->to('admin/reviewer')->with('error', 'Error: ' . $e->getMessage());
        }
    }
    ...
    public function assignCategory()
    {
        ...
        if (!$reviewerId || !$categoryId) {
            // 🔁 FIX: redirect eksplisit ke index agar tidak fallback ke "/"
            return redirect()->to('admin/reviewer')->with('error', 'Data tidak lengkap.');
        }
        ...
        if (!$category) {
            return redirect()->to('admin/reviewer')->with('error', 'Kategori tidak ditemukan.');
        }
        ...
        if ($existing) {
            return redirect()->to('admin/reviewer')->with('error', 'Kategori sudah di-assign ke reviewer ini.');
        }

        try {
            $ok = $this->reviewerKategoriModel->save([
                'id_reviewer' => $reviewerId,
                'id_kategori' => $categoryId,
            ]);
            ...
            if ($ok) {
                ...
                return redirect()->to('admin/reviewer')->with('success', 'Kategori berhasil di-assign!');
            }
            return redirect()->to('admin/reviewer')->with('error', 'Gagal assign kategori.');
        } catch (\Exception $e) {
    ...
    {
        if (!$assignmentId) {
            return redirect()->to('admin/reviewer')->with('error', 'ID assignment tidak valid.');
        }

        $assignment = $this->reviewerKategoriModel->find($assignmentId);
        if (!$assignment) {
            return redirect()->to('admin/reviewer')->with('error', 'Assignment kategori tidak ditemukan.');
        }
        ...
        try {
            ...
            if ($this->reviewerKategoriModel->delete($assignmentId)) {
                $this->logActivity(
                    session('id_user'),
                    "Removed category '" . ($category['nama_kategori'] ?? 'Unknown Category') .
                    "' from reviewer '" . ($reviewer['nama_lengkap'] ?? 'Unknown Reviewer') . "'"
                );
                return redirect()->to('admin/reviewer')->with('success', 'Assignment kategori berhasil dihapus!');
            }
            return redirect()->to('admin/reviewer')->with('error', 'Gagal menghapus assignment kategori.');
        } catch (\Exception $e) {
    ...
    public function delete($id)
    {
        ...
        if ($pendingReviews > 0) {
            return redirect()->to('admin/reviewer')
                ->with('error', "Tidak dapat menghapus reviewer yang memiliki {$pendingReviews} review pending.");
        }
        ...
        try {
            ...
            return redirect()->to('admin/reviewer')->with('error', 'Error menghapus reviewer: ' . $e->getMessage());
        }
    }
    ...
    public function detail($id)
    {
        ...
        try {
            ...
            $assignedIds        = array_column($categories, 'id_kategori');
            $availableCategories= $this->kategoriModel;
            if (!empty($assignedIds)) $availableCategories = $availableCategories->whereNotIn('id_kategori', $assignedIds);
    ...
    public function export()
    {
        try {
            ...
            fputcsv($output, ['ID','Nama Lengkap','Email','Status','Total Review','Pending Review','Completion Rate','Kategori']);

            foreach ($reviewers as $r) {
                $cats = array_column($r['categories'], 'nama_kategori');
                fputcsv($output, [
                    $r['id_user'],
                    $r['nama_lengkap'],
                    $r['email'],
            ...
            return redirect()->to('admin/reviewer')->with('error', 'Gagal export data reviewer.');
        }
    }
    ...
    public function getStatistics()
    {
        try {
            ...
            for ($i = 5; $i >= 0; $i--) {
                ...
                $monthlyTrends[] = ['month' => $monthName, 'reviews' => $count];
            }
    ...
    private function getTopPerformers($limit = 5)
    {
        ...
        foreach ($reviewers as $r) {
            $p = $this->getReviewerPerformance($r['id_user']);
            $performers[] = [
                'name'            => $r['nama_lengkap'],
