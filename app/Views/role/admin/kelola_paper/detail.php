 ...
 * Ringkas: tanpa "0/3 review masuk", tombol: Tugaskan Abstrak & Tugaskan Full Paper.
 */
...
$mapAbsStatus = function (?string $s): array {
  ...
  if ($sl === 'sedang_direview') return ['Review','bg-primary-subtle'];
    ...
    <div class="container-xxl px-3 px-md-4 py-4" id="kelola-presenter">
      ...
      <div class="hero-blue card-glass mb-3 p-3 p-md-4 d-flex justify-content-between align-items-start gap-3">
        <div>
          ...
          <div class="text-white-75 small">Ringkasan penugasan Abstrak & Full Paper.</div>
        </div>
          ...
          <?php else: ?>
            <div class="table-responsive">
              <table class="table align-middle kp-table">
                <thead>
                  <tr>
                    <th style="width:36%">Author & Paper</th>
                    <th class="text-center" style="width:16%">Reviewer Abstrak</th>
                    <th class="text-center" style="width:20%">Reviewer Full Paper</th>
                ...
                <?php foreach ($presenters as $p):
                  ...
                  [$absText,$absClass] = $mapAbsStatus($p['status'] ?? null);
                  [$fpText ,$fpClass ] = $mapFpStatus($p['full_paper_status'] ?? null);
                  ...
                  <tr class="kp-row">
                    ...
                    <td class="text-center">
                      <?php if ($absAssigned > 0): ?>
                        <span class="badge bg-success-subtle">Ditugaskan (<?= $absAssigned ?>)</span>
                      <?php else: ?>
                        <span class="badge bg-secondary-subtle">Belum ditugaskan</span>
                      <?php endif; ?>
                    </td>

                    <!-- Reviewer Full Paper (tanpa "0/3 review masuk") -->
                    <td class="text-center">
                      <span class="badge bg-primary-subtle"><?= min(3,$fpAssigned) ?>/3 reviewer</span>
                    </td>

                    <!-- Status Abstrak -->
                    <td class="text-center">
                      <span class="badge <?= esc($absClass) ?>"><?= esc($absText) ?></span>
                      <?php if(!$hasAbs): ?><div class="tiny text-muted">Belum unggah</div><?php endif; ?>
                    </td>

                    <!-- Status Full -->
                    <td class="text-center">
                      <span class="badge <?= esc($fpClass) ?>"><?= esc($fpText) ?></span>
                      <?php if(!$hasFull): ?><div class="tiny text-muted">Belum unggah</div><?php endif; ?>
                    </td>
                    ...
                    <td class="text-end">
                      <div class="d-flex flex-wrap justify-content-end gap-2">
                        <a class="btn btn-outline-primary btn-sm" href="<?= esc($assignAbs) ?>">
                          <i class="bi bi-person-plus me-1"></i>Tugaskan Abstrak
                        </a>
                        <a class="btn btn-primary btn-sm" href="<?= esc($assignFp) ?>">
                          <i class="bi bi-people me-1"></i>Tugaskan Full Paper
                        </a>
                      </div>
                    </td>
                  </tr>
...
.kp-table thead th{ background:#f8fafc!important; border:0; padding:.9rem .9rem; font-weight:800; color:#0f172a; }
.kp-table tbody tr.kp-row{ background:#fff; border:1px solid rgba(30,64,175,.12); box-shadow:0 8px 18px rgba(30,64,175,.08); }
...
.kp-cell-author .kp-clip-2{
  display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; max-height:3rem;
}
