                ...
                <div class="fp-card js-card" data-search="<?= esc($c['searchable']) ?>">
                  ...
                  <?php if (!empty($c['hint'])): ?>
                    <div class="mini-hint mt-2">
                      <i class="bi bi-info-circle me-1"></i><?= esc($c['hint']) ?>
                    </div>
                  <?php endif; ?>

                  <div class="fp-foot">
                    <?php if (!empty($c['can_upload'])): ?>
                      <a class="btn btn-primary flex-fill" href="<?= esc($c['primary_url']) ?>">
                        <i class="bi bi-upload me-1"></i><?= esc($c['primary_text']) ?>
                      </a>
                    <?php endif; ?>
                    <?php if (!empty($c['detail_url'])): ?>
                      <a class="btn btn-outline-secondary flex-fill" href="<?= esc($c['detail_url']) ?>">
            ...
            <?php else: foreach ($history as $h): ?>
              <div class="col">
                <div class="fp-card js-card" data-search="<?= esc($h['searchable']) ?>">
                  ...
                  <div class="chip-row mb-2">
                    <?php if (!empty($h['kategori'])): ?>
                      <span class="chip alt"><i class="bi bi-tag me-1"></i><?= esc($h['kategori']) ?></span>
                    <?php endif; ?>
                    <?php if (!empty($h['uploaded_at']) && $h['uploaded_at']!=='-'): ?>
                      <span class="chip"><i class="bi bi-cloud-arrow-up me-1"></i><?= esc($h['uploaded_at']) ?></span>
                    <?php endif; ?>
                  </div>
                  ...
                  <div class="fp-foot">
                    <?php if (!empty($h['detail_url'])): ?>
                      <a class="btn btn-outline-primary btn-sm w-100" href="<?= esc($h['detail_url']) ?>">
                        <i class="bi bi-eye me-1"></i>Detail
                      </a>
                    <?php endif; ?>
                  </div>
...
.fp-card:hover{
  ...
  box-shadow:0 16px 28px rgba(30,64,175,.16);
...
.fp-foot{
  ...
  margin-top:auto;
  padding-top:.6rem;
  border-top:1px dashed rgba(30,64,175,.16);
}
.btn{
  font-weight:800;
  border-radius:10px;
