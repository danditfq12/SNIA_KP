...
$S           = $submission ?? [];
$taskStatus  = strtolower($taskStatus ?? 'accepted');
...
// sekarang detail hanya untuk tugas yg SUDAH accepted
$canReview = true;
          ...
          <div class="card card-glass border-0 mb-3">
            <div class="card-header d-flex align-items-center justify-content-between bg-transparent border-0 pb-0">
              ...
              <div class="d-flex gap-2">
                <button class="btn btn-outline-secondary btn-sm" type="button" id="btnTogglePdf" <?= $hasFile ? '' : 'disabled' ?>>
    ...
    <form method="post" action="<?= site_url('reviewer/fullpaper/submit/'.(int)$S['id']) ?>" class="modal-content" id="reviewForm">
      ...
      <div class="modal-body">
        <div class="mb-3">
          ...
          <select name="keputusan" class="form-select" <?= !$hasFile ? 'disabled' : '' ?> required>
        ...
        <div>
          ...
          <textarea name="komentar" rows="6" class="form-control" <?= !$hasFile ? 'disabled' : '' ?> placeholder="Tulis komentar yang jelas dan konstruktif..."><?= esc($my['komentar'] ?? '') ?></textarea>
          <div class="form-text">Komentar <b>wajib diisi</b>. Jika memilih <b>Ditolak</b>, minimal 10 karakter.</div>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-light" type="button" data-bs-dismiss="modal">Batal</button>
        <button class="btn btn-primary" type="submit" <?= !$hasFile ? 'disabled' : '' ?> id="btnSubmitReview">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', (e) => {
    if (!select || !text || select.disabled || text.disabled) return;
