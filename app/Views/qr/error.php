...
class QRAttendance extends BaseController
{
    ...
    public function showScannerInterface()
    {
        $data = [
            ...
            'base_url' => base_url(),
            'user_name' => session('nama_lengkap') ?? 'Guest',
            'is_logged_in' => session()->has('id_user'),
    ...
    public function scan($qrToken = null)
    {
        ...
        // Initialize arrays for user data if logged in
        $attendanceHistory = [];
        $verifiedPayments = [];
        ...
        if ($isLoggedIn) {
            ...
            $verifiedPayments = $this->pembayaranModel
                ->select('pembayaran.*, events.title as event_title, events.event_date, events.event_time, events.location')
            ...
            $hasVerifiedPayment = !empty($verifiedPayments);
        }
        ...
        $data = [
            ...
            'title' => 'QR Attendance - ' . ($event['title'] ?? 'SNIA Event'),
            
            // User-specific data (for presenter/audience views)
            ...
            'absensi' => $attendanceHistory,
            'currentEvent' => null,
            'eventOngoing' => $eventStatus['can_attend'] ?? false,
            'eventMessage' => $eventStatus['message'] ?? ''
        ];
    ...
    // Continue with the rest of the methods from the previous version...
    // [Rest of the methods remain the same as in the previous complete version]
